...
void testRotateTmpl(const double phase)
{
    ...
    //load the feeder
    auto buffIn = Pothos::BufferChunk(typeid(std::complex<Type>), 10);
    ...
    POTHOS_TEST_EQUAL(buffOut.length, 10*sizeof(std::complex<Type>));
    ...
    for (size_t i = 0; i < 10; i++)
    {
        ...
        POTHOS_TEST_TRUE(std::abs(pOut[i].real() - expected.real()) <= 1);
        POTHOS_TEST_TRUE(std::abs(pOut[i].imag() - expected.imag()) <= 1);
    }
}
