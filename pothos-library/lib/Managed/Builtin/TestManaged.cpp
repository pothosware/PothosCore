...
static void test_simple_runner(Pothos::ProxyEnvironment::Sptr env)
{
    ...
    //test field access
    superBarInstance0.call("_bar", 321);
    POTHOS_TEST_EQUAL(superBarInstance0.call<int>("_bar"), 321);
...
POTHOS_TEST_BLOCK("/proxy/managed/tests", test_remote_node)
{
    //start a server to talk with
    Pothos::RemoteServer server("tcp://0.0.0.0");

    //make a node for this server
    Pothos::RemoteNode node("tcp://localhost:"+server.getActualPort());
    node.communicate();
    POTHOS_TEST_TRUE(node.isOnline());
    POTHOS_TEST_TRUE(not node.getId().empty());
    POTHOS_TEST_TRUE(not node.getName().empty());
    Poco::URI uri(node.getUri());
    POTHOS_TEST_EQUAL(uri.getHost(), "localhost");
    POTHOS_TEST_EQUAL(uri.getPort(), atoi(server.getActualPort().c_str()));

    //save this node and look it up
    node.addToRegistry();

    //check that we find it in the list
    auto keys = Pothos::RemoteNode::listRegistryKeys();
    auto it = std::find(keys.begin(), keys.end(), node.getKey());
    POTHOS_TEST_TRUE(it != keys.end());

    //look up the node by id
    Pothos::RemoteNode lookedupNode = Pothos::RemoteNode::fromKey(node.getKey());
    POTHOS_TEST_TRUE(lookedupNode == node);

    //remove from the list
    node.removeFromRegistry();
    keys = Pothos::RemoteNode::listRegistryKeys();
    it = std::find(keys.begin(), keys.end(), node.getKey());
    POTHOS_TEST_TRUE(it == keys.end());

    //test some fails
    POTHOS_TEST_THROWS(Pothos::RemoteNode::fromKey("bogus"), Pothos::RemoteNodeError);
    POTHOS_TEST_THROWS(Pothos::RemoteNode("tcp://localtoast:12345").makeClient(), Pothos::RemoteNodeError);
}
