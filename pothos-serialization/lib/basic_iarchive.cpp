...
#include <Pothos/serialization/state_saver.hpp>
...
using namespace Pothos::serialization;
...
class basic_iarchive_impl {
    ...
    basic_iarchive_impl(unsigned int flags) :
        m_archive_library_version(POTHOS_ARCHIVE_VERSION()),
        m_flags(flags)
    {}
    ~basic_iarchive_impl(){}
...
//public:
    ...
    const basic_pointer_iserializer * load_pointer(
        ...
        const basic_pointer_iserializer * (*finder)(
            const Pothos::serialization::extended_type_info & type
        )
...
basic_iarchive_impl::register_type(
    ...
    if(result.second){
        ...
        POTHOS_ASSERT(cobject_info_set.size() == cobject_id_vector.size());
    }
...
basic_iarchive_impl::load_object(
    ...
    Pothos::serialization::state_saver<object_id_type> ss_start(m_moveable_objects.start);
...
basic_iarchive_impl::load_pointer(
    ...
    if(class_id_type(cobject_info_set.size()) <= cid){
        ...
        || bpis_ptr->get_basic_serializer().is_polymorphic()){
            ...
            char key[POTHOS_SERIALIZATION_MAX_KEY_SIZE];
        ...
        POTHOS_ASSERT(NULL != bpis_ptr);
        class_id_type new_cid = register_type(bpis_ptr->get_basic_serializer());
        int i = cid;
        cobject_id_vector[i].bpis_ptr = bpis_ptr;
        POTHOS_ASSERT(new_cid == cid);
    }
    int i = cid;
    cobject_id & co = cobject_id_vector[i];
...
POTHOS_ARCHIVE_DECL(const basic_pointer_iserializer *)
...
POTHOS_ARCHIVE_DECL(Pothos::archive::library_version_type) 
basic_iarchive::get_library_version() const{
    return pimpl->m_archive_library_version;
}

POTHOS_ARCHIVE_DECL(unsigned int) 
