...
#include <istream>
#include <algorithm>
#include <Pothos/serialization/impl/config.hpp> // BOOST_DEDUCED_TYPENAME
...
// spirit stuff
#include <Pothos/serialization/impl/spirit/include/classic_operators.hpp>
#include <Pothos/serialization/impl/spirit/include/classic_actions.hpp>
#include <Pothos/serialization/impl/spirit/include/classic_numerics.hpp>
...
#include <Pothos/serialization/impl/function.hpp>
...
#include <Pothos/archive/iterators/xml_unescape.hpp>

using namespace Pothos::spirit::classic;
...
struct append_string {
    ...
    #if 0
        typedef Pothos::archive::iterators::xml_unescape<Iterator> translator;
        contents.append(
            translator(POTHOS_MAKE_PFTO_WRAPPER(start)), 
            translator(POTHOS_MAKE_PFTO_WRAPPER(end))
        );
    #endif
...
struct append_char {
    ...
    void operator()(const unsigned int char_value) const {
        const POTHOS_DEDUCED_TYPENAME String::value_type z = char_value;
        contents += z;
    }
...
struct append_lit {
    ...
    void operator()(const X & /*x*/, const Y & /*y*/) const {
        const POTHOS_DEDUCED_TYPENAME String::value_type z = c;
        contents += z;
    }
...
bool basic_xml_grammar<CharType>::my_parse(
    POTHOS_DEDUCED_TYPENAME basic_xml_grammar<CharType>::IStream & is,
    const rule_t & rule_,
    CharType delimiter
) const {
    if(is.fail()){
    ...
    Pothos::io::ios_flags_saver ifs(is);
    is >> std::noskipws;
    ...
    CharType val;
    do{
        POTHOS_DEDUCED_TYPENAME basic_xml_grammar<CharType>::IStream::int_type
            result = is.get();
        if(is.fail())
            return false;
    ...
    parse_info<POTHOS_DEDUCED_TYPENAME std::basic_string<CharType>::iterator> 
        result = Pothos::spirit::classic::parse(arg.begin(), arg.end(), rule_);
    return result.hit;
}
...
basic_xml_grammar<CharType>::basic_xml_grammar(){
    ...
    CharData =  
        CharDataChars [
            xml::append_string<
                StringType, 
                POTHOS_DEDUCED_TYPENAME std::basic_string<CharType>::const_iterator
            >(rv.contents)
        ]
    ;
    ...
    ClassIDAttribute = 
        str_p(POTHOS_ARCHIVE_XML_CLASS_ID()) >> NameTail
        >> Eq 
        >> L'"'
    ...
    ObjectIDAttribute = (
        str_p(POTHOS_ARCHIVE_XML_OBJECT_ID()) 
        | 
        str_p(POTHOS_ARCHIVE_XML_OBJECT_REFERENCE()) 
        )
        >> NameTail
        >> Eq 
        >> L'"'
    ...
    ClassNameAttribute = 
        str_p(POTHOS_ARCHIVE_XML_CLASS_NAME()) 
        >> Eq 
        >> L'"'
        >> ClassName
        >> L'"'
    ...
    TrackingAttribute = 
        str_p(POTHOS_ARCHIVE_XML_TRACKING())
        >> Eq
        >> L'"'
    ...
    VersionAttribute = 
        str_p(POTHOS_ARCHIVE_XML_VERSION())
        >> Eq
        >> L'"'
...
void basic_xml_grammar<CharType>::init(IStream & is){
    ...
    if(! std::equal(rv.class_name.begin(), rv.class_name.end(), POTHOS_ARCHIVE_SIGNATURE()))
        Pothos::serialization::throw_exception(
