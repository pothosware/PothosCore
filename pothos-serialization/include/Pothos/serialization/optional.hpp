...
#ifndef POTHOS_SERIALIZATION_OPTIONAL_HPP_
#define POTHOS_SERIALIZATION_OPTIONAL_HPP_
...
void save(
    Archive & ar, 
    const Pothos::optional< T > & t, 
    const unsigned int /*version*/
){
    const bool tflag = t.is_initialized();
    ar << Pothos::serialization::make_nvp("initialized", tflag);
    if (tflag){
        const Pothos::serialization::item_version_type item_version(version< T >::value);
        #if 0
        ...
        if(Pothos::archive::library_version_type(3) < library_version){
            ...
            ar << POTHOS_SERIALIZATION_NVP(item_version);
        #endif
        ar << Pothos::serialization::make_nvp("value", *t);
    }
}
...
void load(
    ...
    ar >> Pothos::serialization::make_nvp("initialized", tflag);
    if (tflag){
        Pothos::serialization::item_version_type item_version(0);
        ...
        ar >> Pothos::serialization::make_nvp("value", aux.reference());
...
struct implementation_level<optional< T > >
{
    ...
    typedef mpl::int_<Pothos::serialization::object_serializable> type;
    POTHOS_STATIC_CONSTANT(
        int , 
        value = Pothos::serialization::implementation_level::type::value
    );
};
...
struct tracking_level<optional< T > >
{
    ...
    POTHOS_STATIC_CONSTANT(
        int , 
        value = Pothos::serialization::tracking_level::type::value
    );
};
