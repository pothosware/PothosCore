...
#ifndef POTHOS_MATH_PROMOTION_HPP
#define POTHOS_MATH_PROMOTION_HPP
...
#include <Pothos/serialization/impl/type_traits/is_floating_point.hpp> // for boost::is_floating_point;
#include <Pothos/serialization/impl/type_traits/is_integral.hpp> // for boost::is_integral
#include <Pothos/serialization/impl/type_traits/is_convertible.hpp> // for boost::is_convertible
#include <Pothos/serialization/impl/type_traits/is_same.hpp>// for boost::is_same
#include <Pothos/serialization/impl/type_traits/remove_cv.hpp>// for boost::remove_cv
// Boost Template meta programming:
#include <Pothos/serialization/impl/mpl/if.hpp> // for boost::mpl::if_c.
#include <Pothos/serialization/impl/mpl/and.hpp> // for boost::mpl::if_c.
#include <Pothos/serialization/impl/mpl/or.hpp> // for boost::mpl::if_c.
#include <Pothos/serialization/impl/mpl/not.hpp> // for boost::mpl::if_c.
    ...
    namespace tools
    {
      ...
      { // Promote, if necessary, & pick the wider of the two floating-point types.
        ...
        typedef typename mpl::if_<
          ...
          typename mpl::if_< typename mpl::and_<mpl::not_<is_floating_point<T2P> >, ::Pothos::is_convertible<T1P, T2P> >, T2P, T1P>::type>::type type;
      }; // promote_arg2
      // These full specialisations reduce mpl::if_ usage and speed up
      // compilation:
      ...
      struct promote_args
      {
         ...
         //
         POTHOS_STATIC_ASSERT_MSG((0 == ::Pothos::is_same<type, long double>::value), "Sorry, but this platform does not have sufficient long double support for the special functions to be reliably implemented.");
#endif
      };
