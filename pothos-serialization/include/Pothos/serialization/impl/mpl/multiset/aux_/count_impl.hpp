...
#ifndef POTHOS_MPL_MULTISET_AUX_COUNT_IMPL_HPP_INCLUDED
#define POTHOS_MPL_MULTISET_AUX_COUNT_IMPL_HPP_INCLUDED
...
#include <Pothos/serialization/impl/mpl/count_fwd.hpp>
...
struct multiset_count_impl
    : int_< sizeof(S::key_count(POTHOS_MPL_AUX_STATIC_CAST(U*,0))) - 1 >
{
};
...
{
    typedef U (* u_)();
    typedef int_< sizeof(S::ref_key_count(POTHOS_MPL_AUX_STATIC_CAST(u_,0))) - 1 > type_;
    POTHOS_STATIC_CONSTANT(int, value = type_::value);
    typedef type_ type;
};
}
...
struct count_impl< aux::multiset_tag >
{
    template< typename Set, typename Key > struct apply
    {
        enum { msvc71_wknd_ = sizeof(Set::key_count(POTHOS_MPL_AUX_STATIC_CAST(aux::type_wrapper<Key>*,0))) - 1 };
        typedef int_< msvc71_wknd_ > type;
        POTHOS_STATIC_CONSTANT(int, value = msvc71_wknd_);
    };
};
