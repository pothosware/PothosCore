...
template<
    ...
    >
struct POTHOS_PP_CAT(vector,i_)
    : v_item<
          POTHOS_PP_CAT(T,POTHOS_PP_DEC(i_))
        , AUX778076_VECTOR_TAIL(vector,POTHOS_PP_DEC(i_),T)
        >
{
    typedef POTHOS_PP_CAT(vector,i_) type;
};
#endif
...
#   define AUX778076_VECTOR_ITEM(unused, i_, unused2) \
    typedef POTHOS_PP_CAT(T,i_) POTHOS_PP_CAT(item,i_); \
    /**/

    POTHOS_PP_REPEAT(i_, AUX778076_VECTOR_ITEM, unused)
#   undef AUX778076_VECTOR_ITEM
    typedef void_ POTHOS_PP_CAT(item,i_);
    typedef POTHOS_PP_CAT(T,POTHOS_PP_DEC(i_)) back;
...
template<>
struct push_front_impl< aux::vector_tag<POTHOS_PP_DEC(i_)> >
{
    template< typename Vector, typename T > struct apply
    {
        typedef POTHOS_PP_CAT(vector,i_)<
              T
              POTHOS_PP_COMMA_IF(POTHOS_PP_DEC(i_))
              POTHOS_PP_ENUM_PARAMS(POTHOS_PP_DEC(i_), typename Vector::item)
            > type;
    };
};
...
struct pop_front_impl< aux::vector_tag<i_> >
{
    ...
    {
        typedef POTHOS_PP_CAT(vector,POTHOS_PP_DEC(i_))<
              POTHOS_PP_ENUM_SHIFTED_PARAMS(i_, typename Vector::item)
            > type;
    };
};
...
template<>
struct push_back_impl< aux::vector_tag<POTHOS_PP_DEC(i_)> >
{
    template< typename Vector, typename T > struct apply
    {
        typedef POTHOS_PP_CAT(vector,i_)<
...
struct v_at<V,i_>
{
    typedef typename V::POTHOS_PP_CAT(item,i_) type;
};
...
template<> struct v_at_impl<i_>
{
    ...
    {
        typedef typename V_::POTHOS_PP_CAT(item,i_) type;
    };
};
}
...
struct at_impl< aux::vector_tag<i_> >
{
    ...
    {
        typedef typename aux::v_at_impl<POTHOS_MPL_AUX_VALUE_WKND(N)::value>
