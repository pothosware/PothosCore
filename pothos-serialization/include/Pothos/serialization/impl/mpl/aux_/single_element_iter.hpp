...
#ifndef POTHOS_MPL_AUX_SINGLE_ELEMENT_ITER_HPP_INCLUDED
#define POTHOS_MPL_AUX_SINGLE_ELEMENT_ITER_HPP_INCLUDED
...
template< typename T, POTHOS_MPL_AUX_NTTP_DECL(int, is_last_) >
struct sel_iter;
...
template< typename T, POTHOS_MPL_AUX_NTTP_DECL(int, is_last_), typename Distance >
struct advance< aux::sel_iter<T,is_last_>,Distance>
{
    typedef aux::sel_iter<
          T
        , ( is_last_ + POTHOS_MPL_AUX_NESTED_VALUE_WKND(int, Distance) )
        > type;
};

template< 
      typename T
    , POTHOS_MPL_AUX_NTTP_DECL(int, l1)
    , POTHOS_MPL_AUX_NTTP_DECL(int, l2) 
    >
...
template<> struct distance_impl<aux::sel_iter_tag>
{
    template< typename Iter1, typename Iter2 > struct apply
    {
        ...
        POTHOS_STATIC_CONSTANT(int, value = ( pos2_ - pos1_ ));
    };
};
