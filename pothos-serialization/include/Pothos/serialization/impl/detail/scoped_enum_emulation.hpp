...
scoped enums with C++03 namespaces and enums. The Boost.Config POTHOS_NO_CXX11_SCOPED_ENUMS
...
The user can declare  declare these types as
  ...
  POTHOS_SCOPED_ENUM_DECLARE_BEGIN(future_errc)
  {
      broken_promise,
      future_already_retrieved,
      promise_already_satisfied,
  ...
  POTHOS_SCOPED_ENUM_DECLARE_END(future_errc)
...
use
        ...
        switch (Pothos::native_value(ev))
        {
        case future_errc::broken_promise:

And instead of

    #ifdef POTHOS_NO_CXX11_SCOPED_ENUMS
    template <>
    struct POTHOS_SYMBOL_VISIBLE is_error_code_enum<future_errc> : public true_type { };
    #endif
...
use
    ...
    #ifdef POTHOS_NO_CXX11_SCOPED_ENUMS
    template <>
    struct POTHOS_SYMBOL_VISIBLE is_error_code_enum<future_errc::enum_type > : public true_type { };
    #endif


Sample usage:

     POTHOS_SCOPED_ENUM_UT_DECLARE_BEGIN(algae, char) { green, red, cyan }; POTHOS_SCOPED_ENUM_DECLARE_END(algae)
     ...
 ...
 Light
  ...
  Sample usage:
     ...
     POTHOS_SCOPED_ENUM_START(algae) { green, red, cyan }; POTHOS_SCOPED_ENUM_END
     ...
     POTHOS_SCOPED_ENUM(algae) sample( algae::red );
     void foo( POTHOS_SCOPED_ENUM(algae) color );
     ...
     sample = algae::green;
     foo( algae::cyan );
...
#ifndef POTHOS_SCOPED_ENUM_EMULATION_HPP
#define POTHOS_SCOPED_ENUM_EMULATION_HPP
...
#ifndef POTHOS_NO_CXX11_EXPLICIT_CONVERSION_OPERATORS

#define POTHOS_SCOPED_ENUM_UT_DECLARE_CONVERSION_OPERATOR \
 ...
 */
#define POTHOS_SCOPED_ENUM_UT_DECLARE_BEGIN(EnumType, UnderlyingType)    \
    struct EnumType {                                                   \
        typedef UnderlyingType underlying_type;                         \
        EnumType() POTHOS_NOEXCEPT {}                                    \
        explicit EnumType(underlying_type v) : v_(v) {}                 \
        underlying_type get_underlying_value_() const { return v_; }               \
        POTHOS_SCOPED_ENUM_UT_DECLARE_CONVERSION_OPERATOR                \
    private:                                                            \
        underlying_type v_;                                             \
...
#define POTHOS_SCOPED_ENUM_DECLARE_END2() \
        enum_type get_native_value_() const POTHOS_NOEXCEPT { return enum_type(v_); } \
        operator enum_type() const POTHOS_NOEXCEPT { return get_native_value_(); } \
        friend bool operator ==(self_type lhs, self_type rhs) POTHOS_NOEXCEPT { return enum_type(lhs.v_)==enum_type(rhs.v_); } \
        friend bool operator ==(self_type lhs, enum_type rhs) POTHOS_NOEXCEPT { return enum_type(lhs.v_)==rhs; } \
        friend bool operator ==(enum_type lhs, self_type rhs) POTHOS_NOEXCEPT { return lhs==enum_type(rhs.v_); } \
        friend bool operator !=(self_type lhs, self_type rhs) POTHOS_NOEXCEPT { return enum_type(lhs.v_)!=enum_type(rhs.v_); } \
        friend bool operator !=(self_type lhs, enum_type rhs) POTHOS_NOEXCEPT { return enum_type(lhs.v_)!=rhs; } \
        friend bool operator !=(enum_type lhs, self_type rhs) POTHOS_NOEXCEPT { return lhs!=enum_type(rhs.v_); } \
        friend bool operator <(self_type lhs, self_type rhs) POTHOS_NOEXCEPT { return enum_type(lhs.v_)<enum_type(rhs.v_); } \
...
#define POTHOS_SCOPED_ENUM_DECLARE_END(EnumType) \
    ; \
    EnumType(enum_type v) POTHOS_NOEXCEPT : v_(v) {}                 \
    POTHOS_SCOPED_ENUM_DECLARE_END2()
 ...
 * @param EnumType The new scoped enum.
 */
#define POTHOS_SCOPED_ENUM_DECLARE_BEGIN(EnumType) \
  POTHOS_SCOPED_ENUM_UT_DECLARE_BEGIN(EnumType,int)
 ...
 */
#define POTHOS_SCOPED_ENUM_NATIVE(EnumType) EnumType::enum_type
/**
 * Forward declares an scoped enum.
 *
 * @param NT The scoped enum.
 */
#define POTHOS_SCOPED_ENUM_FORWARD_DECLARE(EnumType) struct EnumType

#else  // BOOST_NO_CXX11_SCOPED_ENUMS

#define POTHOS_SCOPED_ENUM_UT_DECLARE_BEGIN(EnumType,UnderlyingType) enum class EnumType:UnderlyingType
#define POTHOS_SCOPED_ENUM_DECLARE_BEGIN(EnumType) enum class EnumType
#define POTHOS_SCOPED_ENUM_DECLARE_END2()
#define POTHOS_SCOPED_ENUM_DECLARE_END(EnumType) ;

#define POTHOS_SCOPED_ENUM_NATIVE(EnumType) EnumType
#define POTHOS_SCOPED_ENUM_FORWARD_DECLARE(EnumType) enum class EnumType

#endif  // BOOST_NO_CXX11_SCOPED_ENUMS

#define POTHOS_SCOPED_ENUM_START(name) POTHOS_SCOPED_ENUM_DECLARE_BEGIN(name)
#define POTHOS_SCOPED_ENUM_END POTHOS_SCOPED_ENUM_DECLARE_END2()
#define POTHOS_SCOPED_ENUM(name) POTHOS_SCOPED_ENUM_NATIVE(name)
