#ifndef POTHOS_TT_IS_ABSTRACT_CLASS_HPP
#define POTHOS_TT_IS_ABSTRACT_CLASS_HPP
...
#ifdef POTHOS_IS_ABSTRACT
template <class T>
struct is_abstract_imp
{
   POTHOS_STATIC_CONSTANT(bool, value = POTHOS_IS_ABSTRACT(T));
};
#elif !defined(POTHOS_NO_IS_ABSTRACT)
template<class T>
...
#ifdef __GNUC__
   POTHOS_STATIC_CONSTANT(std::size_t, s1 = sizeof(is_abstract_imp2<T>::template check_sig<T>(0)));
#else
#if POTHOS_WORKAROUND(POTHOS_MSVC_FULL_VER, >= 140050000)
#pragma warning(push)
#pragma warning(disable:6334)
#endif
   POTHOS_STATIC_CONSTANT(std::size_t, s1 = sizeof(check_sig<T>(0)));
#if POTHOS_WORKAROUND(POTHOS_MSVC_FULL_VER, >= 140050000)
#pragma warning(pop)
#endif
#endif
...
struct is_abstract_imp
{
   typedef is_abstract_select< ::Pothos::is_class<T>::value> selector;
   typedef typename selector::template rebind<T> binder;
   typedef typename binder::type type;

   POTHOS_STATIC_CONSTANT(bool, value = type::value);
};
...
POTHOS_TT_AUX_BOOL_TRAIT_DEF1(is_abstract,T,::Pothos::detail::is_abstract_imp<T>::value)
#else
POTHOS_TT_AUX_BOOL_TRAIT_DEF1(is_abstract,T,::Pothos::detail::is_polymorphic_imp<T>::value)
#endif
