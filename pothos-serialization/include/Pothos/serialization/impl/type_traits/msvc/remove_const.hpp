...
#ifndef POTHOS_TYPE_TRAITS_MSVC_REMOVE_CONST_HOLT_2004_0828
#define POTHOS_TYPE_TRAITS_MSVC_REMOVE_CONST_HOLT_2004_0828
...
namespace Pothos {
    namespace detail {
        ...
        struct remove_const_impl_typeof<true,false,true,false> {
            ...
            struct inner {
                ...
                static msvc_register_type<T,ID> test(...);
                POTHOS_STATIC_CONSTANT(unsigned,register_test=sizeof(test( (void(*)(T))(NULL) ) ));
                typedef typename msvc_extract_type<ID>::id2type::type type;
            };
            template<typename T>
            struct transform_type {
        ...
        struct remove_const_impl_typeof<false,true,true,false> {
            template<typename T,typename ID>
            struct inner {
                POTHOS_STATIC_CONSTANT(unsigned,value=(sizeof(T)/sizeof((*((T*)NULL))[0])));
    ...
    template<typename T>
    struct remove_const {
        typedef Pothos::detail::remove_const_impl_typeof<
            Pothos::is_pointer<T>::value,
            Pothos::is_array<T>::value,
            Pothos::is_const<T>::value,
        ...
        typedef typename 
            ...
            >::type
        type;
        POTHOS_MPL_AUX_LAMBDA_SUPPORT(1,remove_const,T)
    };
}//namespace boost
