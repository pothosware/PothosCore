...
#ifndef POTHOS_TT_IS_ENUM_HPP_INCLUDED
#define POTHOS_TT_IS_ENUM_HPP_INCLUDED

#include <Pothos/serialization/impl/type_traits/intrinsics.hpp>
#ifndef POTHOS_IS_ENUM
...
#if defined(POTHOS_TT_HAS_CONFORMING_IS_CLASS_IMPLEMENTATION) 
...
struct is_class_or_union
{
# if POTHOS_WORKAROUND(POTHOS_MSVC, < 1300) || POTHOS_WORKAROUND(__BORLANDC__, POTHOS_TESTED_AT(0x581))// we simply can't detect it this way.
    POTHOS_STATIC_CONSTANT(bool, value = false);
# else
    template <class U> static ::Pothos::type_traits::yes_type is_class_or_union_tester(void(U::*)(void));

#  if POTHOS_WORKAROUND(POTHOS_MSVC, == 1300)                 \
    || POTHOS_WORKAROUND(__MWERKS__, <= 0x3000) // no SFINAE
    static ::Pothos::type_traits::no_type is_class_or_union_tester(...);
    POTHOS_STATIC_CONSTANT(
        bool, value = sizeof(is_class_or_union_tester(0)) == sizeof(::Pothos::type_traits::yes_type));
#  else
    template <class U>
    static ::Pothos::type_traits::no_type is_class_or_union_tester(...);
    POTHOS_STATIC_CONSTANT(
        bool, value = sizeof(is_class_or_union_tester<T>(0)) == sizeof(::Pothos::type_traits::yes_type));
#  endif
# endif
};
#endif
...
{
    template <typename T> struct type
       : public ::Pothos::is_convertible<typename Pothos::add_reference<T>::type,::Pothos::detail::int_convertible>
    {
    };
};
...
template <typename T> struct is_enum_impl
{
   ...
   // (dwa 2002/04/15)...
   POTHOS_STATIC_CONSTANT(bool, selector =
      (::Pothos::type_traits::ice_or<
           ::Pothos::is_arithmetic<T>::value
         , ::Pothos::is_reference<T>::value
         , ::Pothos::is_function<T>::value
   ...
   POTHOS_STATIC_CONSTANT(bool, selector =
    ...
    typedef ::Pothos::detail::is_enum_helper<
          ::Pothos::detail::is_enum_impl<T>::selector
        > se_t;
#else
    typedef ::Pothos::detail::is_enum_helper<selector> se_t;
#endif

    typedef typename se_t::template type<T> helper;
    POTHOS_STATIC_CONSTANT(bool, value = helper::value);
};

// these help on compilers with no partial specialization support:
POTHOS_TT_AUX_BOOL_TRAIT_IMPL_SPEC1(is_enum,void,false)
#ifndef POTHOS_NO_CV_VOID_SPECIALIZATIONS
POTHOS_TT_AUX_BOOL_TRAIT_IMPL_SPEC1(is_enum,void const,false)
POTHOS_TT_AUX_BOOL_TRAIT_IMPL_SPEC1(is_enum,void volatile,false)
POTHOS_TT_AUX_BOOL_TRAIT_IMPL_SPEC1(is_enum,void const volatile,false)
#endif

} // namespace detail

POTHOS_TT_AUX_BOOL_TRAIT_DEF1(is_enum,T,::Pothos::detail::is_enum_impl<T>::value)

#else // __BORLANDC__
//
// buggy is_convertible prevents working
// implementation of is_enum:
POTHOS_TT_AUX_BOOL_TRAIT_DEF1(is_enum,T,false)

#endif

#else // BOOST_IS_ENUM

POTHOS_TT_AUX_BOOL_TRAIT_DEF1(is_enum,T,POTHOS_IS_ENUM(T))
