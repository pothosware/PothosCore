...
#ifndef POTHOS_TT_IS_POLYMORPHIC_HPP
#define POTHOS_TT_IS_POLYMORPHIC_HPP

#include <Pothos/serialization/impl/type_traits/intrinsics.hpp>
#ifndef POTHOS_IS_POLYMORPHIC
...
{
# if POTHOS_WORKAROUND(__MWERKS__, <= 0x2407) // CWPro7 should return false always.
...
#  endif 
   ...
   struct d2 : public ncvT
   {
      d2();
      virtual ~d2()throw();
#  if !defined(POTHOS_MSVC) && !defined(__ICL)
...
# endif 
   POTHOS_STATIC_CONSTANT(bool, value = (sizeof(d2) == sizeof(d1)));
};
...
{
   typedef is_polymorphic_selector< ::Pothos::is_class<T>::value> selector;
   typedef typename selector::template rebind<T> binder;
   typedef typename binder::type imp_type;
   POTHOS_STATIC_CONSTANT(bool, value = imp_type::value);
};

} // namespace detail

POTHOS_TT_AUX_BOOL_TRAIT_DEF1(is_polymorphic,T,::Pothos::detail::is_polymorphic_imp<T>::value)

#else // BOOST_IS_POLYMORPHIC

POTHOS_TT_AUX_BOOL_TRAIT_DEF1(is_polymorphic,T,POTHOS_IS_POLYMORPHIC(T))
