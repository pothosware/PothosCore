#ifndef POTHOS_SERIALIZATION_SINGLETON_HPP
#define POTHOS_SERIALIZATION_SINGLETON_HPP
...
{
private:
    POTHOS_DLLEXPORT static T & instance;
    // include this to provoke instantiation at pre-execution time
    static void use(T const &) {}
    POTHOS_DLLEXPORT static T & get_instance() {
        static detail::singleton_wrapper< T > t;
        // refer to instance, causing it to be instantiated (and
        // initialized at startup on working compilers)
        POTHOS_ASSERT(! detail::singleton_wrapper< T >::m_is_destroyed);
        use(instance);
        return static_cast<T &>(t);
    }
public:
    POTHOS_DLLEXPORT static T & get_mutable_instance(){
        POTHOS_ASSERT(! is_locked());
        return get_instance();
    }
    POTHOS_DLLEXPORT static const T & get_const_instance(){
        return get_instance();
    }
    POTHOS_DLLEXPORT static bool is_destroyed(){
        return detail::singleton_wrapper< T >::m_is_destroyed;
    }
};

template<class T>
POTHOS_DLLEXPORT T & singleton< T >::instance = singleton< T >::get_instance();
