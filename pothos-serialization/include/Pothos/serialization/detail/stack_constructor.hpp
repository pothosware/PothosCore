#ifndef  POTHOS_SERIALIZATION_DETAIL_STACH_CONSTRUCTOR_HPP
#define POTHOS_SERIALIZATION_DETAIL_STACH_CONSTRUCTOR_HPP
...
#include <Pothos/serialization/impl/aligned_storage.hpp>
...
private:
    typedef POTHOS_DEDUCED_TYPENAME Pothos::aligned_storage<
        sizeof(T), 
        #if POTHOS_WORKAROUND(__BORLANDC__,POTHOS_TESTED_AT(0x560))
            8
        #else
            Pothos::alignment_of<T>::value
        #endif
    > type;
    type storage_;
};
...
struct stack_construct : public stack_allocate<T>
{
    stack_construct(Archive & ar, const unsigned int version){
        ...
        Pothos::serialization::load_construct_data_adl(
            ar, 
            this->address(), 
            version
        );
