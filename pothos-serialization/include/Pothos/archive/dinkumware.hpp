#ifndef POTHOS_ARCHIVE_DINKUMWARE_HPP
#define POTHOS_ARCHIVE_DINKUMWARE_HPP
...
operator<<(basic_ostream<CharType> & os, Pothos::uint64_t t){
...
template<class CharType>
basic_ostream<CharType> & 
operator<<(basic_ostream<CharType> &os, Pothos::int64_t t){
    if(0 <= t){
        os << static_cast<Pothos::uint64_t>(t);
    }
    else{
        os.put('-');
        os << -t;
...
operator>>(basic_istream<CharType> &is, Pothos::int64_t & t){
    CharType d;
    do{
        d = is.get();
    }
...
template<class CharType>
basic_istream<CharType> & 
operator>>(basic_istream<CharType> &is, Pothos::uint64_t & t){
    Pothos::int64_t it;
    is >> it;
    t = it;
    return is;
