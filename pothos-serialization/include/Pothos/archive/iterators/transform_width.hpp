#ifndef POTHOS_ARCHIVE_ITERATORS_TRANSFORM_WIDTH_HPP
#define POTHOS_ARCHIVE_ITERATORS_TRANSFORM_WIDTH_HPP
...
#include <Pothos/serialization/impl/config.hpp> // for BOOST_DEDUCED_TYPENAME & PTFO
...
template<
    ...
    int BitsOut, 
    int BitsIn, 
    class CharType = POTHOS_DEDUCED_TYPENAME Pothos::iterator_value<Base>::type // output character
>
class transform_width : 
    public Pothos::iterator_adaptor<
    ...
    >
{
    friend class Pothos::iterator_core_access;
    typedef POTHOS_DEDUCED_TYPENAME Pothos::iterator_adaptor<
        transform_width<Base, BitsOut, BitsIn, CharType>,
        Base,
        CharType,
        single_pass_traversal_tag,
    ...
    typedef POTHOS_DEDUCED_TYPENAME iterator_value<Base>::type base_value_type;
...
public:
    ...
    template<class T>
    transform_width(POTHOS_PFTO_WRAPPER(T) start) : 
        super_t(Base(POTHOS_MAKE_PFTO_WRAPPER(static_cast< T >(start)))),
        m_buffer_out_full(false),
        m_remaining_bits(0),
