#ifndef POTHOS_ARCHIVE_ADD_FACET_HPP
#define POTHOS_ARCHIVE_ADD_FACET_HPP
...
#    define POTHOS_ARCHIVE_OLD_DINKUMWARE_BENEATH_STLPORT
#  endif
#endif
...
add_facet(const std::locale &l, Facet * f){
    return
        #if defined POTHOS_ARCHIVE_OLD_DINKUMWARE_BENEATH_STLPORT 
            // std namespace used for native locale
            new std::locale(std::_Addfac(l, f));
        #elif POTHOS_WORKAROUND(POTHOS_DINKUMWARE_STDLIB, == 1) // old Dinkumwar
            // std namespace used for native locale
            new std::locale(std::_Addfac(l, f));
        #else
            // standard compatible
...
#undef POTHOS_ARCHIVE_OLD_DINKUMWARE_BENEATH_STLPORT
