...
#include <Pothos/archive/basic_binary_oprimitive.hpp>
...
#define POLYMORPHIC(T) Pothos::archive::detail::polymorphic_oarchive_route<T>
...
namespace eos {
	...
	typedef Pothos::archive::basic_binary_oprimitive<
		portable_oarchive
	...
	class portable_oarchive : public portable_oprimitive
		...
		, public Pothos::archive::basic_binary_oarchive<portable_oarchive>
	...
	#endif
	{
		...
		void init(unsigned flags)
		{
			...
			if (flags & Pothos::archive::no_header)
				POTHOS_ASSERT(archive_version == 3);
			else
			{
...
//				save<unsigned>(archive_version);
	...
	public:
		...
		#if POTHOS_VERSION < 103400
			: portable_oprimitive(os, flags & Pothos::archive::no_codecvt)
		#else
			: portable_oprimitive(*os.rdbuf(), flags & Pothos::archive::no_codecvt)
		#endif
			, Pothos::archive::basic_binary_oarchive<portable_oarchive>(flags)
		{
			init(flags);
		}
		...
		portable_oarchive(std::streambuf& sb, unsigned flags = 0)
			: portable_oprimitive(sb, flags & Pothos::archive::no_codecvt)
			, Pothos::archive::basic_binary_oarchive<portable_oarchive>(flags)
		{
			init(flags);
		}
         ...
         * you should check your POTHOS_CLASS_IMPLEMENTATION setting for A, as
		...
		save(const T & t, dummy<2> = 0)
		{
			if (T temp = t)
			{
				...
				POTHOS_ASSERT(t > 0 || Pothos::is_signed<T>::value);
		...
		save(const T& t, dummy<4> = 0)
		{
			...
			save((typename Pothos::uint_t<sizeof(T)*CHAR_BIT>::least)(t));
		}
	};
...
// required by export
#if POTHOS_VERSION < 103500
#define POTHOS_ARCHIVE_CUSTOM_OARCHIVE_TYPES eos::portable_oarchive
#else
POTHOS_SERIALIZATION_REGISTER_ARCHIVE(eos::portable_oarchive)
POTHOS_SERIALIZATION_REGISTER_ARCHIVE(eos::polymorphic_portable_oarchive)
#endif
