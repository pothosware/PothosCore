 ...
 * platform Pothos::serialization user. While being almost as fast as the native
...
#define POLYMORPHIC(T) Pothos::archive::detail::polymorphic_iarchive_route<T>

// endian and fpclassify
#include <Pothos/serialization/impl/spirit/home/support/detail/endian/endian.hpp>
#include <Pothos/serialization/impl/spirit/home/support/detail/math/fpclassify.hpp>

// namespace alias
#if POTHOS_VERSION < 103800
namespace fp = Pothos::math;
#else
namespace fp = Pothos::spirit::math;
#endif

// namespace alias endian
#if POTHOS_VERSION < 104800
namespace endian = Pothos::detail;
#else
namespace endian = Pothos::spirit::detail;
#endif
...
#include <Pothos/serialization/impl/program_options/detail/convert.hpp>
#endif
...
namespace eos {
	...
	typedef Pothos::archive::basic_binary_iprimitive<
		portable_iarchive
	#if POTHOS_VERSION < 103400
		, std::istream
	#else
	 ...
	 * Pothos::uint64_t in your serialization structures.
	 *
	...
	class portable_iarchive : public portable_iprimitive
		...
		, public Pothos::archive::basic_binary_iarchive<portable_iarchive>

	#if POTHOS_VERSION >= 103500
		// mix-in helper class for serializing shared_ptr
		, public Pothos::archive::detail::shared_ptr_helper
	#endif
	{
		// only needed for Robert's hack in basic_binary_iarchive::init
		friend class Pothos::archive::basic_binary_iarchive<portable_iarchive>;
		...
		{
			using namespace Pothos::archive;
	...
	public:
		 ...
		 * the Pothos::serialization standard archive header containing also the
		 * library version. Due to efficiency we stick with our own.
		 */
		portable_iarchive(std::istream& is, unsigned flags = 0)
		#if POTHOS_VERSION < 103400
			: portable_iprimitive(is, flags & Pothos::archive::no_codecvt)
		#else
			: portable_iprimitive(*is.rdbuf(), flags & Pothos::archive::no_codecvt)
		#endif
			, Pothos::archive::basic_binary_iarchive<portable_iarchive>(flags)
		{
			init(flags);
		}
		...
		portable_iarchive(std::streambuf& sb, unsigned flags = 0)
			: portable_iprimitive(sb, flags & Pothos::archive::no_codecvt)
			, Pothos::archive::basic_binary_iarchive<portable_iarchive>(flags)
		{
			init(flags);
		}
         ...
         * should check your POTHOS_CLASS_IMPLEMENTATION setting for A, as
		 ...
		 */
		template <typename T>
		typename Pothos::enable_if<Pothos::is_integral<T> >::type
		load(T & t, dummy<2> = 0)
		{
			...
			if (signed char size = load_signed_char())
			{
				...
				if (size < 0 && Pothos::is_unsigned<T>::value)
		 ...
		 */
		template <typename T>
		typename Pothos::enable_if<Pothos::is_floating_point<T> >::type
		load(T & t, dummy<3> = 0)
		{
			...
			POTHOS_STATIC_ASSERT(sizeof(bits) == sizeof(T));
			POTHOS_STATIC_ASSERT(std::numeric_limits<T>::is_iec559);
		...
		template <typename T>
		typename Pothos::disable_if<Pothos::is_arithmetic<T> >::type
		load(T& t, dummy<4> = 0)
		{
			...
			load((typename Pothos::uint_t<sizeof(T)*CHAR_BIT>::least&)(t));
		}
	};
...
#if POTHOS_VERSION < 103500
#define POTHOS_ARCHIVE_CUSTOM_IARCHIVE_TYPES eos::portable_iarchive
#else
POTHOS_SERIALIZATION_REGISTER_ARCHIVE(eos::portable_iarchive)
POTHOS_SERIALIZATION_REGISTER_ARCHIVE(eos::polymorphic_portable_iarchive)
#endif
...
#include <Pothos/archive/impl/basic_binary_iarchive.ipp>
#include <Pothos/archive/impl/basic_binary_iprimitive.ipp>

#if !defined POTHOS_ARCHIVE_SERIALIZER_INCLUDED
#include <Pothos/archive/impl/archive_serializer_map.ipp>
#define POTHOS_ARCHIVE_SERIALIZER_INCLUDED
#endif
