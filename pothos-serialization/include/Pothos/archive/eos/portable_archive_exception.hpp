...
#include <Pothos/serialization/impl/lexical_cast.hpp>
...
namespace eos {
	...
	#if POTHOS_VERSION < 104400
		typedef Pothos::archive::version_type archive_version_type;
	#else
		typedef Pothos::archive::library_version_type archive_version_type;
	#endif

	// version of the linked boost archive library
	const archive_version_type archive_version(
	#if POTHOS_VERSION < 103700
		Pothos::archive::ARCHIVE_VERSION()
	#else
		Pothos::archive::POTHOS_ARCHIVE_VERSION()
	#endif
	);
	 ...
	 */
	class portable_archive_exception : public Pothos::archive::archive_exception
	{
		std::string msg;
	...
	public:
		...
		portable_archive_exception(signed char invalid_size) 
			: Pothos::archive::archive_exception(other_exception) 
			, msg("requested integer size exceeds type size: ")
		{
			msg += Pothos::lexical_cast<std::string, int>(invalid_size);
		}
		...
		{
			msg += Pothos::lexical_cast<std::string>(abnormal);
		}
