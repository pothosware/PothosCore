...
POTHOS_TEST_BLOCK("/blocks/tests", test_proxy_subtopology)
{
    ...
    auto feeder = registry.callProxy("/blocks/sources/feeder_source", "int");
    auto collector = registry.callProxy("/blocks/sinks/collector_sink", "int");
    auto forwarder = registry.callProxy("/blocks/misc/forwarder", "int");
    ...
    auto subtopology = env->findProxy("Pothos/Topology").callProxy("new");
    subtopology.callProxy("connect", subtopology, "0", forwarder, "0");
    subtopology.callProxy("connect", forwarder, "0", subtopology, "0");
    subtopology.call("disconnectAll");

    //run the topology
    std::cout << "run the topology\n";
    {
        auto topology = env->findProxy("Pothos/Topology").callProxy("new");
        topology.callProxy("connect", feeder, "0", subtopology, "0");
        topology.callProxy("connect", subtopology, "0", collector, "0");
        topology.callProxy("commit");
        POTHOS_TEST_TRUE(topology.call<bool>("waitInactive"));
    }
