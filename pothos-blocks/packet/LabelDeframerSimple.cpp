 ...
 * |widget StringEntry()
 *
 * |factory /blocks/labeldeframersimple()
 * |setter setPacketLength(packetLength)
 * |setter setFrameStartLabel(frameStartLabel)
 **********************************************************************/
class LabelDeframerSimple : public Pothos::Block
{
public:

    static Block *make(void)
    {
        return new LabelDeframerSimple();
    }

    LabelDeframerSimple(void) : packetLength(0x1), frameStartLabel("Matched!")
    {
        this->setupInput(0, typeid(unsigned char));
        this->setupOutput(0, typeid(unsigned char));
        this->registerCall(this, POTHOS_FCN_TUPLE(LabelDeframerSimple, setPacketLength));
        this->registerCall(this, POTHOS_FCN_TUPLE(LabelDeframerSimple, setFrameStartLabel));
    }
...
static Pothos::BlockRegistry registerLabelDeframerSimple(
    "/blocks/labeldeframersimple", &LabelDeframerSimple::make);
