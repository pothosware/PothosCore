...
#include <Pothos/serialization/impl/program_options/detail/utf8_codecvt_facet.hpp>
#include <Pothos/serialization/impl/throw_exception.hpp>

#include <Pothos/serialization/impl/bind.hpp>
...
namespace Pothos { namespace detail {
    ...
    /* Internal function to actually perform conversion.
       ...
       function will pass functional objects created with Pothos::bind.
...
namespace Pothos {
    ...
    POTHOS_PROGRAM_OPTIONS_DECL std::wstring 
    from_8_bit(const std::string& s, 
               const std::codecvt<wchar_t, char, std::mbstate_t>& cvt)
    {
        return detail::convert<wchar_t>(
            s,                 
            Pothos::bind(&std::codecvt<wchar_t, char, mbstate_t>::in,
                        &cvt,
                        _1, _2, _3, _4, _5, _6, _7));
    }

    POTHOS_PROGRAM_OPTIONS_DECL std::string 
    to_8_bit(const std::wstring& s, 
             const std::codecvt<wchar_t, char, std::mbstate_t>& cvt)
    {
        return detail::convert<char>(
            s,                 
            Pothos::bind(&codecvt<wchar_t, char, mbstate_t>::out,
                        &cvt,
                        _1, _2, _3, _4, _5, _6, _7));
    }
    ...
    namespace {
        Pothos::program_options::detail::utf8_codecvt_facet
            utf8_facet;
    }
    ...
    from_local_8_bit(const std::string& s)
    {
        ...
        return from_8_bit(s, 
                          POTHOS_USE_FACET(facet_type, locale()));
    }
    ...
    {
        POTHOS_PROGRAM_OPTIONS_DECL std::string to_internal(const std::string& s)
        {
            return s;
        }

#ifndef POTHOS_NO_STD_WSTRING
        POTHOS_PROGRAM_OPTIONS_DECL std::string to_internal(const std::wstring& s)
        {
            return to_utf8(s);
        }
#endif
