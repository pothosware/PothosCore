...
shared_ptr_helper::append(const Pothos::shared_ptr<const void> &sp){
    // make tracking array if necessary
    if(NULL == m_pointers)
        m_pointers = new collection_type;
    ...
    if(i == m_pointers->end()){
        ...
        POTHOS_ASSERT(result.second);
    }
}

//  #ifdef BOOST_SERIALIZATION_SHARED_PTR_132_HPP
POTHOS_ARCHIVE_DECL(void)
shared_ptr_helper::append(const Pothos_132::shared_ptr<const void> & t){
    if(NULL == m_pointers_132)
        m_pointers_132 = new std::list<Pothos_132::shared_ptr<const void> >;
