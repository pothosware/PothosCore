...
#ifndef POTHOS_MATH_FP_TRAITS_HPP
#define POTHOS_MATH_FP_TRAITS_HPP
...
#   define POTHOS_FPCLASSIFY_VAX_FORMAT
#endif
...
#           define POTHOS_FPCLASSIFY_PREFIX ::__std_alias:: 
#        else
#           define POTHOS_FPCLASSIFY_PREFIX ::_STLP_VENDOR_CSTD:: 
#        endif
#     else
#        define POTHOS_FPCLASSIFY_PREFIX ::std::
#     endif
#  else
#     undef POTHOS_HAS_FPCLASSIFY
#     define POTHOS_FPCLASSIFY_PREFIX
#  endif
...
#ifndef POTHOS_FPCLASSIFY_VAX_FORMAT

template<> struct fp_traits_non_native<float, single_precision>
{
    typedef ieee_copy_all_bits_tag method;

    POTHOS_STATIC_CONSTANT(uint32_t, sign        = 0x80000000u);
    POTHOS_STATIC_CONSTANT(uint32_t, exponent    = 0x7f800000);
    POTHOS_STATIC_CONSTANT(uint32_t, flag        = 0x00000000);
    POTHOS_STATIC_CONSTANT(uint32_t, significand = 0x007fffff);
...
#if defined(POTHOS_NO_INT64_T) || defined(POTHOS_NO_INCLASS_MEMBER_INITIALIZATION) \
...
template<> struct fp_traits_non_native<double, double_precision>
{
    ...
    POTHOS_STATIC_CONSTANT(uint32_t, sign        = 0x80000000u);
    POTHOS_STATIC_CONSTANT(uint32_t, exponent    = 0x7ff00000);
    POTHOS_STATIC_CONSTANT(uint32_t, flag        = 0);
    POTHOS_STATIC_CONSTANT(uint32_t, significand = 0x000fffff);
...
#if defined(POTHOS_BIG_ENDIAN)
    POTHOS_STATIC_CONSTANT(int, offset_ = 0);
#elif defined(POTHOS_LITTLE_ENDIAN)
    POTHOS_STATIC_CONSTANT(int, offset_ = 4);
#else
    POTHOS_STATIC_ASSERT(false);
#endif
};
...
struct fp_traits_non_native<long double, extended_double_precision>
{
    ...
    POTHOS_STATIC_CONSTANT(uint32_t, sign        = 0x80000000u);
    POTHOS_STATIC_CONSTANT(uint32_t, exponent    = 0x7fff0000);
    POTHOS_STATIC_CONSTANT(uint32_t, flag        = 0x00008000);
    POTHOS_STATIC_CONSTANT(uint32_t, significand = 0x00007fff);
...
private:
    ...
    POTHOS_STATIC_CONSTANT(int, offset_ = 0);
#elif defined(POTHOS_LITTLE_ENDIAN)
    POTHOS_STATIC_CONSTANT(int, offset_ = 12);
#else
    POTHOS_STATIC_ASSERT(false);
#endif
};
...
struct fp_traits_non_native<long double, extended_double_precision>
{
    ...
    POTHOS_STATIC_CONSTANT(uint32_t, exponent    = 0x7fff0000);
    POTHOS_STATIC_CONSTANT(uint32_t, flag        = 0x00000000);
    POTHOS_STATIC_CONSTANT(uint32_t, significand = 0x0000ffff);
...
{
    typedef POTHOS_DEDUCED_TYPENAME size_to_precision<sizeof(T), ::Pothos::is_floating_point<T>::value>::type precision;
...
#if (defined(POTHOS_MATH_USE_C99) && !(defined(__GNUC__) && (__GNUC__ < 4))) \
   && !defined(__hpux) \
   && !defined(__DECCXX)\
   && !defined(__osf__) \
   && !defined(__SGI_STL_PORT) && !defined(_STLPORT_VERSION)\
   && !defined(POTHOS_MATH_DISABLE_STD_FPCLASSIFY)
#  define POTHOS_MATH_USE_STD_FPCLASSIFY
#endif

template<class T> struct fp_traits
{
    typedef POTHOS_DEDUCED_TYPENAME size_to_precision<sizeof(T), ::Pothos::is_floating_point<T>::value>::type precision;
#if defined(POTHOS_MATH_USE_STD_FPCLASSIFY) && !defined(POTHOS_MATH_DISABLE_STD_FPCLASSIFY)
