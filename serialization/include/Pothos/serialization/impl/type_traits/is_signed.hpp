...
#ifndef POTHOS_TT_IS_SIGNED_HPP_INCLUDED
#define POTHOS_TT_IS_SIGNED_HPP_INCLUDED
...
{
   typedef typename remove_cv<T>::type no_cv_t;
   POTHOS_STATIC_CONSTANT(bool, value = (!(::Pothos::detail::is_signed_values<T>::minus_one  > Pothos::detail::is_signed_values<T>::zero)));
};
...
struct is_signed_imp
{
   ...
   POTHOS_STATIC_CONSTANT(bool, value = is_signed_imp<T>::type::value);
#else
...
#if defined( __CODEGEARC__ )
POTHOS_TT_AUX_BOOL_TRAIT_DEF1(is_signed,T,__is_signed(T))
#else
POTHOS_TT_AUX_BOOL_TRAIT_DEF1(is_signed,T,::Pothos::detail::is_signed_imp<T>::value)
#endif
