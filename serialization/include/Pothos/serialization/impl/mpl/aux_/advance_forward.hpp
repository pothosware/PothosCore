...
#ifndef POTHOS_MPL_AUX_ADVANCE_FORWARD_HPP_INCLUDED
#define POTHOS_MPL_AUX_ADVANCE_FORWARD_HPP_INCLUDED
...
#   define POTHOS_MPL_PREPROCESSED_HEADER advance_forward.hpp
...
// forward declaration
template< POTHOS_MPL_AUX_NTTP_DECL(long, N) > struct advance_forward;

#   define POTHOS_PP_ITERATION_PARAMS_1 \
    (3,(0, POTHOS_MPL_LIMIT_UNROLLING, <boost/mpl/aux_/advance_forward.hpp>))
...
struct advance_forward
{
    template< typename Iterator > struct apply
    {
        typedef typename apply_wrap1<
              advance_forward<POTHOS_MPL_LIMIT_UNROLLING>
            , Iterator
            >::type chunk_result_;
...
template<>
struct advance_forward< POTHOS_PP_FRAME_ITERATION(1) >
{
    template< typename Iterator > struct apply
    {
...
#   include POTHOS_PP_ITERATE()
#endif
        typedef POTHOS_PP_CAT(iter,i_) type;
    };
