...
#include <Pothos/archive/wcslen.hpp>
...
void save_iterator(std::ostream &os, InputIterator begin, InputIterator end){
    typedef Pothos::archive::iterators::mb_from_wchar<
        Pothos::archive::iterators::xml_escape<InputIterator>
    > translator;
    std::copy(
        translator(POTHOS_MAKE_PFTO_WRAPPER(begin)), 
        translator(POTHOS_MAKE_PFTO_WRAPPER(end)), 
        Pothos::archive::iterators::ostream_iterator<char>(os)
    );
}
...
//  at least one library doesn't typedef value_type for strings
//  so rather than using string directly make a pointer iterator out of it
    typedef Pothos::archive::iterators::xml_escape<
        const char * 
    > xml_escape_translator;
    std::copy(
        xml_escape_translator(POTHOS_MAKE_PFTO_WRAPPER(s.data())),
        xml_escape_translator(POTHOS_MAKE_PFTO_WRAPPER(s.data()+ s.size())), 
        Pothos::archive::iterators::ostream_iterator<char>(os)
    );
}
...
xml_oarchive_impl<Archive>::save(const char * s){
    ...
    > xml_escape_translator;
    std::copy(
        xml_escape_translator(POTHOS_MAKE_PFTO_WRAPPER(s)),
        xml_escape_translator(POTHOS_MAKE_PFTO_WRAPPER(s + std::strlen(s))), 
        Pothos::archive::iterators::ostream_iterator<char>(os)
    );
}
