...
#include <Pothos/archive/iterators/xml_escape.hpp>
#include <Pothos/archive/iterators/wchar_from_mb.hpp>
#include <Pothos/archive/iterators/ostream_iterator.hpp>
...
void save_iterator(std::wostream &os, InputIterator begin, InputIterator end){
    ...
    > xmbtows;
    std::copy(
        xmbtows(POTHOS_MAKE_PFTO_WRAPPER(begin)),
        xmbtows(POTHOS_MAKE_PFTO_WRAPPER(end)),
        Pothos::archive::iterators::ostream_iterator<wchar_t>(os)
    );
}
...
xml_woarchive_impl<Archive>::save(const std::wstring & ws){
    ...
    std::copy(
        xmbtows(POTHOS_MAKE_PFTO_WRAPPER(ws.begin())),
        xmbtows(POTHOS_MAKE_PFTO_WRAPPER(ws.end())),
        Pothos::archive::iterators::ostream_iterator<wchar_t>(os)
    );
#endif
    typedef iterators::xml_escape<const wchar_t *> xmbtows;
    std::copy(
        xmbtows(POTHOS_MAKE_PFTO_WRAPPER(ws.data())),
        xmbtows(POTHOS_MAKE_PFTO_WRAPPER(ws.data() + ws.size())),
        Pothos::archive::iterators::ostream_iterator<wchar_t>(os)
    );
}
...
xml_woarchive_impl<Archive>::save(const wchar_t * ws){
    ...
    typedef iterators::xml_escape<const wchar_t *> xmbtows;
    std::copy(
        xmbtows(POTHOS_MAKE_PFTO_WRAPPER(ws)),
        xmbtows(POTHOS_MAKE_PFTO_WRAPPER(ws + std::wcslen(ws))),
        Pothos::archive::iterators::ostream_iterator<wchar_t>(os)
    );
}
#endif
    ...
    basic_xml_oarchive<Archive>(flags)
{
    ...
    if(0 == (flags & no_codecvt)){
        Pothos::archive::detail::utf8_codecvt_facet *pfacet;
        #if defined(__SGI_STL_PORT)
            static Pothos::archive::detail::utf8_codecvt_facet 
                facet(static_cast<size_t>(1));
            pfacet = & facet;
        #else
            pfacet = new Pothos::archive::detail::utf8_codecvt_facet;
        #endif
