...
POTHOS_TEST_BLOCK("/util/tests", test_simd_dispatcher_utils_dump_header)
{
    ...
    // Before doing anything, do a sanity check and make sure we're passing in valid JSON.
    std::cout << "Input: " << std::endl << std::endl << nlohmann::json::parse(jsonInputString) << std::endl << std::endl;

    const auto generatedHeaderString = proxy.call<std::string>("generateAndDumpHeader", jsonInputString, testFeatureSet);
    std::cout << "Output: " << std::endl << std::endl << generatedHeaderString << std::endl << std::endl;

    // Make sure the generated header is proper C++ syntax. Since we're not actually calling these functions,
    // we don't need to worry about a linker error.
